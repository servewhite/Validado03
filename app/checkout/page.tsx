...
function CheckoutContent() {
  ...
  // Helper to validate CPF has 11 digits
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        return !!(items.length > 0 && customerData.name && customerData.email && customerData.cpf && customerData.phone && isValidCpf(customerData.cpf))
      case 2:
        return !!(
          cep &&
          addressData.street &&
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      ...
                      <div className="grid gap-1.5 sm:gap-2">
                        ...
                        <Input
                          ...
                          className={`h-10 sm:h-11 text-sm ${customerData.cpf && !isValidCpf(customerData.cpf) ? "border-destructive" : ""}`}
                        />
                        {customerData.cpf && !isValidCpf(customerData.cpf) && (
                          <p className="text-xs text-destructive">CPF deve conter 11 digitos</p>
                        )}
                      </div>
